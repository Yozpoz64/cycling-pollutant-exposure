...
def plot_all_segments():
    ...
    fig.text(0.075, 0.5, 'NO$_2$ (\u00B5g / m$^2$)', va='center', rotation='vertical', fontsize=font)
...
def plot_two_segments():
    ...
    ax[0, 1].set_ylabel('NO$_2$ (\u00B5g / m$^2$)')
    ...
    ax[1, 1].set_ylabel('NO$_2$ (\u00B5g / m$^2$)')
...
def plot_by_road():
    ...
    for layer in layers:
        if layer in layers_of_interest:
            ...
            series = gdf.loc[gdf['full_road_'] == road]
    ...
    ax[0].set_ylabel('Average NO$_2$ (\u00B5g / m$^2$)', fontsize=15)
    ax[0].tick_params(axis='x', labelrotation=45, labelsize=13)
    
    ax[1].legend()
    ax[1].set_title('{}'.format(road))
    ax[1].set_xlabel('Time (Unix Epoch)')
    ax[1].set_ylabel('NO$_2$ (\u00B5g / m$^2$)')
